...
```{r data visualization, include = TRUE, fig.cap = "Fig 1 : Effect of grassland on species richness"}
ggplot(data=df)+
...
```{r, validation of the chosen model with the Bartlett test}
...
```{r, residual normality test with Shapiro}
...
## **1. Visualation of data and one-factor analysis of variance** 
