...
- Selection of the best model "by hand"
```{r full model}
...
```
Exclusion of the variable "island", because it is the non-significant variable with the highest p-value, in the model and test again for variables' significance.
```{r model2}
...
drop1(modlm2,test="F")
```
Exclusion of the variable "clutch_completion", because it is non-significant, in the model and test again for variables' significance.
...
- Selection of the best model with the AIC criterion
```{r model selection AIC}
modfinal<-stepAIC(modlm,direction="both")
summary(modfinal)
```
With this method, the best model includes the variable "clutch completion", however, it's not significant so it confirms our first model selection.

Finaly, the model can be written as :
...
The model explain 88% of the body mass variance.
...
Residuals are normaly distribute.
...
```
There is no influent individual.
