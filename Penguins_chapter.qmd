...
Data were collected in Antarctica on Biscoe, Dream and Torgersen Islands (\@map_antarctica).

```{r}
#| label : map_antarctica
#| fig-cap : "Map of Antarctica and the three islands."
...
gg_zone_interet = ggplot() +
  ...
  geom_sf(data = point_proj, color = c("#a6bddb","#3690c0","#034e7b"), size = 3, shape = 19) +
  coord_sf(crs = "+proj=laea +lat_0=-90 +datum=WGS84") +
  theme_minimal() +
...
box1 <- ggplot(data = data)+
  geom_boxplot(aes(x=species,y=body_mass, fill = species))+
    scale_fill_manual(values = c("#B2DF8A","#FDBF6F","#CAB2D6"))
