...
```{r,echo=TRUE,code.fold=TRUE,code.summary="Show the code"}
...
These three points show the location of the data collect in Antartica.
...
barplot(NA_df, main = "NA quantity per column",
        ylab = "NA", col = NA_col, las=2, cex.names=0.6)
...
The red line represents the mean of NA values in the dataset and the bars are showing the number of NA's for each variables.
...
We can see from this table that the *Adelie* species is present on all three islands, while the *Chrinstrap* and *Gentoo* species are only present on 2 islands, Dream and Biscoe respectively.
...
It would appear that there is a positive linear trend between the culmen_depth variable and culmen_length for each specie.
...
There seems to be different trends between species.
...
@eig_value_plot is an histogram of the variance of the factorial axes with the percentage of inertia of each axis. Here, keep 2 dimensions and 78.4% inertia.

```{r}
#| label: eig_value_plot
...
@contribution_plot Determind the absolut contribution of variables to each axis. We can see that 3 variables make a strong contribution to axis 1 (delta_15_N, body_mass and culmen_depth) and 2 to axis 2 (culmen_depth and delta_13_C) (Figure 2).

```{r,warning=FALSE,message=FALSE, fig.height=2, fig.width=5, fig.align = "center"}
#| label: contribution_plot
