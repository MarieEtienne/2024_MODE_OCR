...
Now, we can check again that no modalities are under-represented (thresold 3%).
...
We can see that the nests are clearly separated by the state they come from. 
We can see a light shift on axis 2 with the nest fate values. The other variables
are forming less distinct groups. 
