...
We will perform an ANOVA to see the effect of qualitatives variables (state, habitat, species, year) on the quantitative Initiation_Date in the Bird_dataset.csv dataset.

We need to check the type of the variables :
  
```{r Ini_factor data, echo=TRUE,include=TRUE}
...
```{r Ini_datahist, include=TRUE, fig.height=5, fig.width=5}
par(mfrow=c(2,2))
# Boxplot
...
```{r Ini_delete_modalites, include=TRUE}
...
```{r Ini_datagraph, include=TRUE, fig.height=3, fig.width=6}
# Boxplot 
par(mfrow=c(2,2))
...
```{r Ini_dataInter, include=TRUE, fig.height=4, fig.width=7}
# Interaction table
...
```{r Ini_candidate_model,include=TRUE}
# Model formulation
...
```{r ini_ResidNorm, include=TRUE, fig.height=4, fig.width=6}
par(mfrow=c(1,2))
# Histogram
...
```{r ini_Residhomo, include=TRUE, fig.height=6, fig.width=8}
par(mfrow=c(2,2))
# residuals vs fitted
...
```{r ini_Contri, include=TRUE, fig.height=4, fig.width=4}
par(mfrow = c(1, 1))
