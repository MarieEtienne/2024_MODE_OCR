...
-   *First question : Does the number of fledged birs depend on the species ?*
...
species <- as.character(unique(data_species$Species))
...
ggcorrplot(
  ...
  scale_fill_gradient2(
    ...
    high = "springgreen3", 
    midpoint = 0.2) +
  ggtitle("P-values of the correlation tests with the number of fledged birds as variable")
...
```{r}
### Formating the dataframe by keeping only the species for the moment. 
data_sp <- as.data.frame(table(data$Species))
data_sp <- as.data.frame(t(data_sp))

### Rename the column of the dataframe
colnames(data_sp) <- dplyr::slice(data_sp,1) # Retrive the first line containing the colnames. 
data_sp <- data_sp[-1,] # Delete the first line. 
