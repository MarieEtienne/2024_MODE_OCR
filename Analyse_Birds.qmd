...
data2 <- data
data2$Initiation_Date <- lapply(X = data2$Initiation_Date, 
                                FUN = num_to_month
                                )
data2$Nest_End_Date <- lapply(X = data2$Nest_End_Date, 
                              FUN = num_to_month
                              )
## Conversion into factors
data2 <- data2 |>
  dplyr::mutate(Nest_Fate = as.character(Nest_Fate),
...
data2$Nest_Fate <- lapply(X = data2$Nest_Fate,
                          FUN = as.character)
```
