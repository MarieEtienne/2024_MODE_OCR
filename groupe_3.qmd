...
Nos données sont issues de l'article "Biogeography at a regional scale : determinants of ant species density in New England bogs and forests" de @Gotelli_2002
...
2. Explorer les données (valeurs aberrantes, distribution des valeurs,) d'après le protocole décrit dans l'article de @Zuur_2010\
...
Une modélisation linéaire généralisée s'écrit comme suit (equation 1) : $$g(\mu_{y})= \alpha+ \beta_{1}.X_{i1}+ \beta_{2}.X_{i2}+\beta_{3}.X_{i3}+...\beta_{p}.X_{ip} = \eta 
$$ {#eq-1}
...
#| echo: true
#| message: false
#| fig-cap: "On test si les titres marchent ou past"
# Boxplot 
...
D'après la  @fig-plot1, $Y$ ne présente pas de valeurs extrêmes ou aberrantes.
