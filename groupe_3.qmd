...
La loi de Poisson est utile pour décrire des évènements discrets rares et indépendants. Ainsi, une variable $Y$ qui suit une loi de Poisson de paramètre $\lambda$ s'écrit comme suit : $$Pr(Y=y)=\frac{e^{-\lambda}.\lambda^y}{y!}$$ où $y$ représente le nombre observé d'occurences (i.e., $y$=0, 1, 2...} et $\lambda$ représente la moyenne et la variance de la distribution de Poisson (i.e. dans la loi de Poisson : E($y$)= Var($y$)=$\lambda$).
...
|||| Les modèles linéaires généralisés de type Poisson ont une structure d'erreur suivant une loi de Poisson. Cette structure d'erreur permet, entre autres, de spécifier correctement la relation entre la moyenne et la variance. Cette relation est utilisée par l'approche du maximum de vraisemblance pour estimer les coefficients et les erreurs standard des paramètres du modèle GLM.||||
