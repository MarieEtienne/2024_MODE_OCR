...
Dans un premier temps, verifier la présence de **surdispersion** est indispensable lorsque l'on fait un modèle linéaire généralisé suivant une loi de Poisson. Pour tous les modèles linéaires généralisées utilisant des données de comptage, il faut vérifier l'absence de surdispersion.

Pour cela, nous allons calculer un paramètre appelé 'scale parameter'. Si ça valeur est supérieure à 1, il y a surdispersion. Le modèle n'est donc pas valable.

### Vérification de la surdispersion

Quand l'index de dispersion est supérieur à 1, il existe différentes explications.

Cela peut être dû à : 
1. Des valeurs abérrantes sont présentes dans le jeu de données. Il faut donc les éliminer. 
2. Des covariables sont manquantes. Il faut les ajouter au modèle. 
3. Des interactions sont manquantes. Il faut les ajouter au modèle. 
4. Une inflation des zéros se produit. Il faut utiliser une regression Binomiale négative à inflation des zéros. https://stats.oarc.ucla.edu/r/dae/zinb/
5. Une dépendance entre les données et un facteur ou une variable est présente. Il faut utiliser un modèle linéaire général mixte.
6.Relation entre les données et les variables ou facteurs n'est pas linéaire. Il faut utiliser un modèle additif généralisé. 
7. La fonction de lien est mauvaise. Il faut changer la fonction de lien.
8. La variation de $Y$ est grande. Il faut utiliser un modèle linéaire généralisé suivant une loi binomiale négative.
