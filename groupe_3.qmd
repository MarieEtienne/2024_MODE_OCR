...
Our data come from the article "Biogeography at a Regional Scale: Determinants of Ant Species Density in New England Bogs and Forests" by Gotelli & Ellison published in *Ecology* in 2002 (83(6), 2002, pp. 1604--1609) (https://doi.org/10.1890/0012-9658(2002)083\[1604:BAARSD\]2.0.CO;2)
...
```{r outliers de Y, echo=TRUE, message=FALSE, warning=FALSE, paged.print=TRUE}
...
(bp + cp) + 
  ...
  plot_annotation(title = "Figure 1 : Analyse of Y outliers")
...
```{r Y distribution, echo=TRUE, message=FALSE, warning=FALSE, paged.print=TRUE}
...
(hg + qqp) +
  plot_layout(widths = c(1, 2)) + 
  plot_annotation(title = "Figure 2 : Analyse of Y distribution")

```

The distribution of $Y$ (negative exponential, Figure 2A) and the nature of the dataset (count data without aggregation) suggest that $Y$ follows a Poisson distribution. This hypothesis is supported by the deviation from normality of $Y$ observed values shown in Figure 2B.
...
p1 <- ggplot(dataAnts) +
  ...
  labs(title = "Figure 3 : Xplanatory variable Latitude",
...
There are no outliers observed in the Cleveland plot (Fig. 3A) or in the histogram (Fig. 3B).

The Quantile plot (Fig. 3C) shows a deviation from normality. This suggests that it is more appropriate to use a generalized linear model (glm) rather than a classical linear model (lm).
...
p4 <- ggplot(dataAnts) +
  ...
  labs(title = "Figure 4 : Explanatory variable Elevation",
       subtitle = "A. Cleveland Plot",
       x= "Elevation") +  
  theme_minimal() +
...
No outliers are observed in the Cleveland plot (Fig. 4A) or in the histogram (Fig. 4B).

The Quantile plot (Fig. 4C) shows a deviation from normality. This suggests that it is advisable to use a generalized linear model rather than a classical linear model.
...
p4 <- ggplot(dataAnts) +
  ...
  labs(title = "Figure 5 : Explanatory variable Area",
       subtitle = "A. Cleveland Plot",
       x= "Area") +  # Titres
  theme_minimal() +
  theme(axis.text.y = element_blank(),   
...
We observe outliers in the Cleveland plot (Fig. 5A) and in the histogram (Fig. 5B) due to the presence of very extensive peatlands. We can apply a transformation to this variable and consider a **logarithmic transformation**.
...
p4 <- ggplot(dataAnts) +
  ...
  labs(title = "Figure 6 : Explanatory variable LogArea",
       subtitle = "A. Cleveland Plot",
       x= "LogArea") +  
  theme_minimal() +
  theme(axis.text.y = element_blank(),   
...
No outliers are observed in the Cleveland plot (Fig. 6A) or in the histogram (Fig. 6B).
...
```{r modalities equilibrium, echo=TRUE, message=FALSE, warning=FALSE, paged.print=TRUE}
...
plot(fitted(mod1)~dataAnts$Nsp,col='dodgerblue3',pch=16,ylab="Estimated values",xlab="Observed values", main="Figure 19")
abline(0,1)
...
-   Zero inflation occurs. Use negative Binomial regression with zero inflation (@Long_J.S._1997)

-   A dependency between the data and a factor or variable is present. A general linear mixed model should be used. (@Johnson_et_al._2015)
...
The histogram of residuals (Fig. 20) and the Q-Q plot (Fig. 21) show relatively normal residuals. The following four graphs (Fig. 22, 23, 24, 25) show the distribution of residuals according to model estimates and the three explanatory variables. We can see that the residuals remain homogeneous whatever the values of the explanatory variables.
...
Here (Fig. 26), there are no overly influential individuals.
...
In our example, the residuals are not overdispersed and show no outliers.
